...
import { getAllClients, registerClient } from './interfaces/clients-response.interface';
...
export class ClientsService {
  ...
  async registerClient(data: CreateClientDto): Promise<registerClient> {
    ...
    if (existToCustomer) {
      return {
        status: 'FAILED',
        message: 'Não foi possível criar o cliente! Tente novamente mais tarde.'
      }
    }
  ...
  async updateClient(_id: string, data: UpdateClientDto): Promise<registerClient> {
    ...
    if(!client) {
      return {
          status: 'FAILED',
          message: 'Não foi possível atualizar o cliente! Tente novamente mais tarde.'
      }
    }
    return {
      status: 'SUCCESS',
  ...
  async removeClient(_id: string) {
    ...
    if(!client) {
      return {
          status: 'FAILED',
          message: 'Não foi possível deletar o cliente! Tente novamente mais tarde.'
      }
    }
    return {
      status: 'SUCCESS',
