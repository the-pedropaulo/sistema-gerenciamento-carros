...
export class CarsService {
    ...
    async getAllCars(_page: number, _limit: number, _name: string, _brand?: string): Promise<getAllCars[]> {
    ...
    async registerCar(data: CreateCarDto): Promise<registerCar> {
        ...
        if(!car) {
            return {
                status: 'FAILED',
                message: 'Não foi possível cadastrar o carro! Tente novamente mais tarde.'
            }
        }
        return {
            status: 'SUCCESS',
    ...
    async updateCar(id_car: string, data: CreateCarDto): Promise<registerCar> {
        ...
        if(!car) {
            return {
                status: 'FAILED',
                message: 'Não foi possível atualizar o carro! Tente novamente mais tarde.'
            }
        }
        return {
            status: 'SUCCESS',
    ...
    async deleteCar(id_car: string): Promise<registerCar> {
        ...
        if(!car) {
            return {
                status: 'FAILED',
                message: 'Não foi possível deletar o carro! Tente novamente mais tarde.'
            }
        }
        return {
            status: 'SUCCESS',
